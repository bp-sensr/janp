...
plot_trend_coef <- function(species) {
  ...
  p2 <- ggplot(coefz_long |> filter(species_code == species), aes(x = term, y = estimate, shape = ecoregion, colour = ecoregion)) +
        ...
        geom_text(aes(label = alpha_sig, y = conf.high),  # anchor to error bar tip
          position = position_dodge(width = 0.8),
          hjust = -0.2,
          vjust = 0.5,
          size = 5,
          colour = "black",          # override inherited colour
          na.rm = TRUE,
          inherit.aes = TRUE) +
        coord_flip() +
