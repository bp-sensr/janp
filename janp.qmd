...
elev_lookup <- wt_get_view("organization_locations", organization = 5344) |>
  filter(grepl('COTTONWOOD|MUSHROOM',locationName)) |>
  select(locationName, elevation) |>
  distinct() |>
  rename(elevation_new = elevation)
...
janp_main <- bind_rows(old_s_max, janp_aru) |>
  ...
  left_join(elev_lookup, by = c("location" = "locationName")) |>
  mutate(elevation = coalesce(elevation, elevation_new)) |>
  select(-elevation_new)
...
#| collapse: true
#| code-fold: true
#| fig-cap: Distribution of locations across elevation gradients. Colours indicate reporting ecoregion for this report.

janp_ready_trend |>
  select(location, latitude, longitude, ecoregion, elevation) |>
  distinct() |>
  mutate(new_ecoregion = case_when(
  ...
  slice(1) |>
  ungroup() |>
  mutate(elevation = case_when(location == "CAVELL-5" ~ 2200, TRUE ~ elevation)) |>
  mutate(ecoregion = factor(ecoregion, levels = ecoregion_order)) |>
  ggplot(aes(x = ecoregion, y = elevation, color = new_ecoregion)) +
  geom_boxplot() +
  geom_jitter(width = 0.15, size = 3, alpha = 0.8) +
  scale_color_manual(values = c("Alpine" = "#7B3F61",
                                "Forested" = "#2E8B57")) +
  theme_bw() +
  theme(
    legend.title = element_text("Ecoregion"),
    plot.title = element_text(face = "bold"),
    panel.grid.minor = element_blank(),
    legend.position = "right"
  ) + ylab("Elevation (m)") + xlab("Ecoregion")
...
[NEW VEGETATION VARIABLES]

@tbl-hab-var

To account for the severe habitat alteration caused by the 2024 Jasper Wildfire, we manually reclassified the land cover variable for all sites on the TEKARRA and VALLEY5 transects **mark** as simply "Burned" for the 2025 and 2026 data **mark** . Assessment of the burn severity classification was not used in this analysis, but field visits and current knowledge would classify the burn as high to severe overall (see also @)
...
#| collapse: true
#| code-fold: true
#| tbl-cap: Proportion of landcover within 150 meter buffer at each location in the Jasper National Park dataset.

hab_var <- janp_ready_trend |>
    mutate(year = factor(year)) |>
    select(location, year, ecoregion, landcover, proportion) |>
    group_by(location, year, ecoregion) |>
    summarise(
        landcovers = paste(unique(paste(landcover, sep=" / ")), collapse="; ")) |>
...
#| collapse: true
#| code-fold: true
#| tbl-cap: Transcription methods by year (2007–2026). Historical data (2007–2020) are undergoing re-transcription to align with the standardized protocol used from 2021 onwards.

transcription_table <- tibble(
  Years = c("2012-2020", "2021-2022", "2007-2011, 2023-2026"),
...
4.  Examined the initial effect of the 2024 Jasper Wildfire on bird community composition;
5.  Modeled temporal changes in functional and community-level diversity; and
6.  Quantified long-term abundance trends (2007–2026), accounting for detection probability and addressing spatial autocorrelation.
...
For alpine model selection, we constructed a candidate model set incorporating observation and detection covariates performed model selection ranked via BIC, which applies a stronger penalty for model complexity than AIC and is appropriate for large candidate model sets [CITATION]. We also determined distribution and zero-inflation of each model for each species and assigning the appropriate type prior to model runs. We scaled all continuous variables including year in order to detect temporal abundance trends. Julian date, hour, detection time, data type (method of data processing) were also included as detection covariates.
...
In the forest model selection, the candidate model set was again ranked via BIC but included the following ecoregion-specific variables: scaled variables for year, shrub cover, conifer cover, wetlands, herbaceous cover, and rock and soil (bedrock, exposed soil). The burned class exclusive to TEKARRA and VALLEY5 locations in 2026 was added to models but created complexity in the models by variance inflation factor testing.
...
comm_matrix1 <- janp_main |>
  mutate(ecoregion = case_when(ecoregion %in% c("Alpine") ~ "Alpine",
                               ecoregion %in% c("Upper Subalpine", "Lower Subalpine", "Montane") ~ "Forested")) |>
  filter(!(grepl("VALLEY5|TEKARRA",location) & year > 2024), year %in% c(2025:2026)) |>
  wt_tidy_species(remove = c("mammal","amphibian","abiotic","insect","unknown"), zerofill = T) |>
...
group <- ifelse(grepl("^COTTONWOOD|^MUSHROOM", rownames(sp_mat)), "Control Transects", "Montane Reference")
...
ggplot(data = t3scores, aes(x = RDA1, y = RDA2)) +
...
shannon_d |>
  mutate(ecoregion = case_when(is.na(ecoregion) ~ "Forested", TRUE ~ ecoregion)) |>
...
The redundancy analysis (RDA) revealed a progressive reorganization of the bird community in the two years following the 2024 wildfire. Reference, 1-year post-fire, and 2-year post-fire survey points occupy increasingly separated regions along RDA1, the axis constrained by years since fire (@fig-after-fire). Rather than shifting once and stabilizing, post-fire sites continued to diverge from Reference conditions between year 1 and year 2, with almost no overlap between the 2-year post-fire ellipse and the Reference ellipse. This indicates that, at least over this short time frame, the bird community has not begun returning toward its pre-fire composition — the reorganization driven by the fire is still deepening rather than reversing. Notably, the spread of sites within each fire period (ellipse size) was similar across all three groups, suggesting that fire has not made site-to-site composition more variable, but has consistently displaced it in a common direction. Species vectors help characterize this directional shift. Dark-eyed junco (*Junco hyemalis*, DEJU) again shows the strongest association with post-fire sites, consistent with its known tolerance of reduced canopy cover and simplified forest structure following disturbance. Swainson's thrush (*Catharus ustulatus*, SWTH), Warbling vireo (*Vireo gilvus*, WAVI), Mountain chickadee (*Poecile gambeli*, MOCH), and Yellow-rumped warbler (*Setophaga coronata*, YRWA) remain most strongly associated with Reference sites, along with Tennessee warbler (*Leiothlypis peregrina*, TEWA), Alder flycatcher (*Empidonax alnorum*, ALFL), Ruby-crowned Kinglet (*Corthylio calendula*, RCKI), and Blackpoll Warbler (*Setophaga striata*, BLPW), all species generally tied to intact canopy cover and vertical foliage structure. Common Yellowthroat (*Geothlypis trichas*, COYE) stood out along PC1 rather than RDA1, indicating its abundance varies with background site-level factors unrelated to time since fire. Together, these results suggest that two years post-fire, the VALLEY5 and TEKARRA bird communities have not shown signs of recovery toward pre-fire composition — the community continues to reorganize around fire-tolerant, open-canopy species such as DEJU, at the expense of canopy-dependent species like SWTH, WAVI, and TEWA.
...
janp_fire_comm <- janp_main |>
  ...
  summarise(individual_order = max(individual_order)) |>
  ungroup() |>
  mutate(years_post_fire = case_when(year < 2025 ~ 0,
                                     year >= 2025 ~ year - 2024),
         fire_period = case_when(year < 2025 ~ "Reference",
                                 year == 2025 ~ "1 year post-fire",
                                 year == 2026 ~ "2 years post-fire",
                                 TRUE ~ NA_character_),
         fire_period = factor(fire_period,
                              levels = c("Reference", "1 year post-fire", "2 years post-fire")))

comm_mat_fire <- janp_fire_comm |>
  pivot_wider(names_from = species_code, values_from = individual_order, values_fill = 0)

env_fire <- comm_mat_fire |>
  select(location, year, years_post_fire, fire_period) |>
  mutate(location = factor(location))

species_mat_fire <- comm_mat_fire |>
  select(-location, -year, -years_post_fire, -fire_period)

species_hell_fire <- decostand(species_mat_fire, method = "hellinger")

rda_fire <- rda(species_hell_fire ~ years_post_fire, data = env_fire)

rda_scores_fire <- scores(rda_fire, display = "sites") |>
  as.data.frame() |>
  rownames_to_column("site") |>
  bind_cols(env_fire)

rda_species_fire <- scores(rda_fire, display = "species") |>
  as.data.frame() |>
  rownames_to_column("species")

ggplot(data = rda_scores_fire, aes(x = RDA1, y = PC1)) +
  geom_point(aes(colour = fire_period), alpha = 0.7, size = 3, shape = 16) +
  stat_ellipse(aes(group = fire_period, colour = fire_period), type = "norm", level = 0.67, linewidth = 1) +
  geom_vline(xintercept = 0, colour = "#A19E99", linetype = 2, linewidth = 0.7) +
  geom_hline(yintercept = 0, colour = "#A19E99", linetype = 2, linewidth = 0.7) +
  geom_segment(data = rda_species_fire, aes(x = 0, y = 0, xend = RDA1, yend = PC1), 
               arrow = arrow(length = unit(0.2, "cm")), linewidth = 0.3, colour = "grey40") +
  geom_text_repel(data = rda_species_fire, aes(x = RDA1, y = PC1, label = species), 
                   size = 3, colour = "black", fontface = "italic", max.overlaps = 10, segment.color = "grey70") +
  theme_bw() +
  scale_colour_viridis_d(option = "cividis", end = 0.9) +
  labs(x = "RDA1", y = "PC1", colour = "Fire period") +
  theme(legend.position = "right", legend.title = element_text(size = 10), legend.text = element_text(size = 9), plot.title = element_text(hjust = 0.5, size = 14, face = "bold"))
...
pdf_fill <- pdf |>
  ...
  tidyr::uncount(4) |>
  group_by(site, class) |>
  mutate(year = 2023:2026) |>
  ungroup()

pdf_all <- bind_rows(
  pdf |> mutate(year = as.integer(as.character(year))),
  pdf_fill
)
...
janp_ready_trend <- janp_main |>
  mutate(equipment_make = case_when(year >= 2023 ~ "Wildlife Acoustics", 
         ...
         equipment_model = case_when(year >= 2023 ~ "SM4",
                                     TRUE ~ "RiverForks")) |>
  left_join(pdf_all, by = c("location" = "site", "year" = "year")) |>
  # Remove single year variables
...
check_zeroinfl <- function(df, response_col, alpha = 0.05) { response_col <- as.character(response_col); count_formula <- as.formula(paste(response_col, "~ 1")); zero_formula <- as.formula(paste(response_col, "~ 1 | 1")); pois_mod <- glm(count_formula, family = poisson, data = df); zip_mod <- pscl::zeroinfl(zero_formula, data = df, dist = "poisson"); out <- capture.output(pscl::vuong(pois_mod, zip_mod)); raw_line <- out[grepl("^Raw", trimws(out))]; if (length(raw_line) == 0) return(FALSE); parts <- strsplit(trimws(raw_line), "\\s+")[[1]]; z_val <- as.numeric(parts[2]); p_val <- as.numeric(tail(parts, 1)); !is.na(z_val) && !is.na(p_val) && p_val < alpha && z_val > 0 }
...
run_trend <- function(spp) {
  ...
  janp_ready <- janp_ready_trend |>
    ...
    ungroup() |>
    mutate(year = year(recording_date_time), hour = hour(recording_date_time), julian = yday(recording_date_time)) |>
    dplyr::select(organization, data_type, n_data_type, project_id, data_type, longitude, latitude, location, elevation,
                  location_id, task_id, ecoregion, landcover, proportion, temperature, wind, recording_date_time, year,
                  hour, julian, observer, species_code, individual_order, abundance, detection_time) |>
    distinct() |>
    wt_tidy_species(remove = c("mammal", "amphibian", "abiotic", "insect", "human", "unknown"), zerofill = TRUE)

  selected_alpine <- janp_ready |>
    filter(ecoregion == "Alpine") |>
    distinct(location, recording_date_time, observer) |>
    group_by(location, recording_date_time) |>
    slice_sample(n = 1) |>
    ungroup()

# Keep all records belonging to the randomly selected observer
janp_alpine <- janp_ready |>
  filter(ecoregion == "Alpine") |>
  semi_join(selected_alpine, by = c("location", "recording_date_time", "observer"))

  base_zero_alpine <- janp_alpine |>
    dplyr::select(location, data_type, elevation, ecoregion, landcover, proportion, temperature, wind,
                  recording_date_time, year, hour, julian, observer, detection_time) |>
    distinct()

  spp_max_alpine <- janp_alpine |>
    filter(species_code == spp) |>
    group_by(location, data_type, elevation, ecoregion, landcover, proportion, temperature, wind, recording_date_time, year, hour, julian, observer, species_code, detection_time) |>
    summarise(individual_order = max(individual_order, na.rm = TRUE)) |>
    ungroup()

  check_spp_alpine <- spp_max_alpine |> filter(species_code == spp) |>
    dplyr::select(location, recording_date_time, species_code, individual_order) |> distinct()
  ...
  if (nrow(check_spp_alpine) <= 3) {
    ...
    jpt_alpine <- base_zero_alpine |>
      left_join(spp_max_alpine, by = c("location", "data_type", "elevation", "ecoregion", "landcover", "proportion",
                                        "temperature", "wind", "recording_date_time", "year", "hour", "julian",
                                        "observer", "detection_time")) |>
      mutate(individual_order = tidyr::replace_na(individual_order, 0)) |>
      filter(!(location == "WILCOX-5" & year == 2022), !is.infinite(individual_order)) |>
      mutate(lc_group = case_when(landcover %in% c("Exposed_Barren_Land", "Rock_Rubble") ~ "Rock and Soil",
                                   landcover %in% c("Wetland-Treed") ~ "Treed Wetland",
                                   TRUE ~ landcover)) |>
      dplyr::select(-c(landcover)) |>
      group_by(location, data_type, elevation, ecoregion, temperature, wind, recording_date_time, observer, individual_order, year, hour, julian, detection_time) |>
      pivot_wider(names_from = lc_group, values_from = proportion, values_fill = 0, values_fn = sum) |>
      ungroup() |>
      mutate(across(c(elevation, Shrubs, Coniferous, Herbs, `Rock and Soil`, julian, hour, temperature, wind, detection_time), ~ as.numeric(scale(.x)))) |>
      mutate(year_c = scale(year)) |>
      distinct()

    print(paste0("Modelling alpine for ", spp))
    fit_alpine <- svabu(individual_order ~ year_c + Shrubs + Coniferous + Herbs | temperature + wind + data_type + detection_time,
                         data = jpt_alpine, zeroinfl = check_zeroinfl(jpt_alpine, "individual_order"))
    summary(fit_alpine)
    ...
    model_fit_alpine <- if (!is.null(fit_alpine)) {
      ...
      z <- est / se
      p <- 2 * pnorm(-abs(z))
      tibble(term = names(est), estimate = est, std.error = se, conf.low = ci[, 1], conf.high = ci[, 2], z.value = z, p.value = p)
    }
  }

  ref_zero_infl_alpine <- jpt_alpine |>
    summarise(julian = mean(julian, na.rm = TRUE), hour = mean(hour, na.rm = TRUE), temperature = mean(temperature, na.rm = TRUE),
              wind = mean(wind, na.rm = TRUE), detection_time = mean(detection_time, na.rm = TRUE))

  newdata_alpine <- jpt_alpine |>
    mutate(julian = ref_zero_infl_alpine$julian, hour = ref_zero_infl_alpine$hour, temperature = ref_zero_infl_alpine$temperature,
           wind = ref_zero_infl_alpine$wind, detection_time = ref_zero_infl_alpine$detection_time)

  jpt_alpine$lambda_hat <- predict(fit_alpine, newdata = newdata_alpine, type = "response")

  lambda_year_alpine <- jpt_alpine |>
    group_by(year) |>
    summarise(n = sum(!is.na(lambda_hat)), se_lh = sd(lambda_hat, na.rm = TRUE) / sqrt(n), lambda_hat = mean(lambda_hat, na.rm = TRUE)) |>
    ungroup() |>
    arrange(year) |>
    dplyr::select(-n) |>
  ...
  lambda_year_alpine_plot <- jpt_alpine |> dplyr::select(year, location, ecoregion, lambda_hat) |> distinct() |> arrange(year)
  ...
  janp_forested <- janp_ready |>
    semi_join(selected_forested, by = c("location", "recording_date_time", "observer")) |>
    mutate(ecoregion = "Forested")
  
  base_zero_forested <- janp_forested |>
    dplyr::select(location, data_type, elevation, ecoregion, landcover, proportion, temperature, wind, recording_date_time, year, hour, julian, observer, detection_time) |>
    distinct()
  ...
  check_spp_forested <- spp_max_forested |> filter(species_code == spp) |>
    dplyr::select(location, recording_date_time, species_code, individual_order) |> distinct()
  ...
  if (nrow(check_spp_forested) <= 3) {
    ...
    jpt_forested <- base_zero_forested |>
      mutate(ecoregion = case_when(ecoregion %in% c("Upper Subalpine", "Lower Subalpine", "Montane") ~ "Forested", TRUE ~ ecoregion)) |>
      left_join(spp_max_forested, by = c("location", "data_type", "elevation", "ecoregion", "landcover", "proportion",
                                          "temperature", "wind", "recording_date_time", "hour", "year", "julian",
                                          "observer", "detection_time")) |>
      mutate(individual_order = tidyr::replace_na(individual_order, 0)) |>
      filter(!(location == "WILCOX-5" & year == 2022), !is.infinite(individual_order)) |>
      mutate(lc_group = case_when(landcover %in% c("Exposed_Barren_Land", "Rock_Rubble") ~ "Rock and Soil",
      ...
      dplyr::select(-c(landcover)) |>
      group_by(location, elevation, ecoregion, temperature, wind, recording_date_time, observer, detection_time, individual_order, year, hour, julian) |>
      pivot_wider(names_from = lc_group, values_from = proportion, values_fill = 0, values_fn = sum) |>
      ungroup() |>
      mutate(across(c(elevation, Shrubs, Coniferous, Broadleaf, Mixedwood, Wetland, julian, hour, temperature, wind, detection_time), scale)) |>
      mutate(year_c = scale(year)) |>
      mutate_at(vars(elevation, Shrubs, Coniferous, Broadleaf, Mixedwood, Wetland, julian, hour, temperature, wind, detection_time, year_c), as.numeric) |>
      distinct() |>
      filter(!is.na(elevation))

    print(paste0("Modelling forested for ", spp))
    fit_forested <- svabu(individual_order ~ year_c + Coniferous + Wetland + elevation | temperature + wind + julian + hour + detection_time + data_type, data = jpt_forested, zeroinfl = check_zeroinfl(jpt_forested, "individual_order"))
    ...
    newdata_forested <- jpt_forested |>
      mutate(julian = ref_zero_infl_forested$julian, hour = ref_zero_infl_forested$hour, temperature = ref_zero_infl_forested$temperature,
             wind = ref_zero_infl_forested$wind, detection_time = ref_zero_infl_forested$detection_time)
  ...
  return(list(bind_rows(trend_result_alpine, trend_result_forested),
              bind_rows(lambda_year_alpine, lambda_year_forested),
              bind_rows(lambda_year_alpine_plot, lambda_year_forested_plot),
              model_fit_alpine, 
              model_fit_forested))
}
...
trendzz <- bind_rows(trendz_new, trendz_new2)
...
plot_trend_coef <- function(species) {
  ...
  p1 <- ggplot(plot_dat_new |> filter(species_code == species) |>
                 group_by(ecoregion, year) |>
                 summarise(mean_lambda = mean(lambda_hat, na.rm = TRUE),
                           se_lambda = sd(lambda_hat, na.rm = TRUE) / sqrt(n())) |>
                 ungroup(), aes(x = year, y = mean_lambda, color = ecoregion)) +
          geom_line() +
          geom_errorbar(aes(ymin = mean_lambda - se_lambda, ymax = mean_lambda + se_lambda), width = 0.3) +
          theme_bw() + 
          scale_colour_viridis_d(option = "cividis") + 
  ...
  p2 <- coefz_long_new |> 
    ...
    scale_colour_manual(values = c(Alpine = "#00204D", Forested = "#C9A400")) +
    scale_fill_manual(values = c(Alpine = "#00204D", Forested = "#C9A400"), na.value = "white") +
    scale_alpha_manual(values = c("***" = 1), na.value = 0.5) +
    geom_hline(yintercept = 0, linetype = "dashed") +
    geom_errorbar(aes(ymin = conf.low, ymax = conf.high, alpha = alpha_sig),
                  position = position_dodge(width = 0.8), width = 0.2) +
    geom_point(aes(fill = significant, alpha = alpha_sig),
...
species_list <- plot_dat_new |> select(species_code) |> distinct() |> slice (1:5) |> pull() 
...
#| fig-cap: Percent change in average abundance trend per species and ecoregion across the full study period. Colours indicate whether the trend represents an increase (>+2.5%), stable (−2.5% to +2.5%), or decrease (<−2.5%) relative to baseline.
#| fig-width: 8
#| fig-height: 12

ggplot(trend_table_new |>
...
species_list2 <- plot_dat_new |> select(species_code) |> distinct() |> arrange(species_code) |> pull()
