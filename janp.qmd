...
Tekarra 9 site photos views from 2023 (A) pre-fire, and 2025 (B) post-fire.
:::
...
@fig-community shows the relationship between species and ecoregion. The PERMANOVA test was performed using Bray-Curtis dissimilarity to assess whether community composition significantly differed between ecoregion groups. The analysis revealed a significant difference in bird community composition between alpine and forest groups (*F*(1,128) = `r round(permanova_result$F[[1]],2)`, *R*² = `r round(permanova_result$R2[[1]],2)`, *p* = `r round(permanova_result$`Pr(>F)`[[1]],2)` indicating a substantial divergence in species composition between ecoregion groups, justifying the decision to analyse trends separately for these two groups.
...
plot_trend_coef <- function(species) {
  ...
  p1 <- ggplot(plot_dat_new |> filter(species_code == species), aes(x = year, y = lambda_hat, color = ecoregion)) + 
          geom_smooth() +
...
species_list <- plot_dat_new |> select(species_code) |> distinct() |> pull() 
